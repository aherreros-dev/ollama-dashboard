...
# float32 is required on MPS: float16 produces NaN in DreamShaper 8's UNet.
# The frontend unloads the Ollama model before generation to free enough RAM.
DTYPE      = torch.float32
PORT       = 7860
...
def _load():
    ...
    _img2img.enable_attention_slicing(1)
    ...
    if step % 5 == 0:
        try:
            ...
            if latents is not None:
                with torch.no_grad():
                    safe_latents = torch.nan_to_num(latents, nan=0.0)
                    decoded = pipe.vae.decode(
                        safe_latents / pipe.vae.config.scaling_factor
                    ).sample
